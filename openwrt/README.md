...
(jffs2 compresses on-flash). The package no longer hard-depends on `openssl-util`:
the binary self-generates the self-signed TLS certificate at startup via pure-Rust
rustls/rcgen, so the old `openssl-util` → (`libopenssl1.1` + `libopenssl-conf`)
dependency chain (~1.3 MB on a clean 22.03 device, and more on 25.x bases where
`libopenssl` is no longer in-base) is reclaimed. On a 16 MB-flash router
(~9.1 MB overlay) that frees roughly an extra ~1.3 MB of overlay headroom after
install — but still check `df -k /overlay` before installing on anything smaller.
Serial kmods
...
- `ca-bundle` - CA trust store for outbound HTTPS to the portal

The self-signed TLS certificate is generated by the binary itself at startup
(pure-Rust rustls/rcgen) when missing, so `openssl-util` is no longer a dependency.
